...
: 확대축소, 회전, 이동을 행렬곱으로 나타내기 위해선 차수가 하나 큰 동차 좌표가 필요하다.  
: 즉 차수를 늘리면 이동까지 선형변환으로 표현할 수 있는데 이런 변환을 아핀 변환이라고 한다.  
: 동차 좌표를 데카르트 좌표계로 표현하려면 w를 추가 축으로 생각하면 된다. 즉 이동을 w축 방향을 포함한 확대 및 회전으로 표현할 수 있다.
- 동차 좌표(homogeneous coordinates)  
: 한 축을 나머지 축의 배수 관계로 차원을 축소하여 생각하는 동차 좌표계는 한 축의 배수에 따라 동종의(homogeneous) 좌표를 얻을 수 있다. 이것이 사영 기하학이다.
* 데카르트 좌표계에서의 선형변환은 새로운 기저(basis)로의 재표현이다.  
따라서 이동은 차원을 늘려 추가적인 기저를 도입해야 표현될 수 있다.
